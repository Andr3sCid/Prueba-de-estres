...
import { IJwtPayload } from './interfaces/IJwtPlayload';
...
function _setJWT(store: IStore, next: Function, action: AnyAction) {
    ...
    if (!Object.keys(actionPayload).length) {
        if (jwt) {
            ...
            try {
                jwtPayload = jwtDecode<IJwtPayload>(jwt); // Usa el tipo aquí
            } catch (e) {
                logger.error(e);
            }

            if (jwtPayload) {
                const { context, iss, sub, name, picture, email } = jwtPayload;

                action.jwt = jwt;
                action.issuer = iss;

                if (context) {
                    const user = _user2participant(
                        context.user as {
            ...
            if (user) {
                _undoOverwriteLocalParticipant(store, user);
            }
        }
    }
