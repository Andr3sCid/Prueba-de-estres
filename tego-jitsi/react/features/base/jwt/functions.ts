...
import { IJwtHeader, IMinimalistJwtPayload } from './interfaces/IJwtPlayload';
...
export function validateJwt(jwt: string) {
    ...
    try {
        const header = jwtDecode<IJwtHeader>(jwt, { header: true });
        const payload = jwtDecode<IMinimalistJwtPayload>(jwt);
...
export function getJwtExpirationDate(jwt: string | undefined) {
    ...
    if (payload) {
        ...
        return new Date(exp as unknown as number * 1000);
    }
}
