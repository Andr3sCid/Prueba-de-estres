...
import { kickParticipant } from '../participants/actions';
...
import { leaveConference } from './actions.any';
...
import { validateSecureCode } from './virtual-participant/API/security';
import { addCaramaAsVirtualParticipant } from './virtual-participant/function';
...
async function _connectionEstablished({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    const token = getState()['features/base/jwt'];
    const localParticipant = getLocalParticipant(getState());

    console.log('Este es el participante local: ', localParticipant);

    // if (token.jwt) {
    //     const access = await validateSecureCode(token.jwt ?? '', getRoomName(getState()) ?? '');

    //     if (!access) {
    //         window.close();
    //         APP.store.dispatch(leaveConference());
    //     }
    // } else {
    //     window.close();
    //     APP.store.dispatch(leaveConference());
    ...
    if (APP !== undefined) {
        await addCaramaAsVirtualParticipant();
    }
